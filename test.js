        ...
        container.addEventListener('wheel', (e) => {
            ...
            if (state.layoutMode === 'timeline') {
                // Timeline: zoom changes dayWidth
...
function getNodeEdgePoint(node, targetNode) {
        ...
        function renderNodes() {
            ...
            function renderChildren(parentNode, containerEl, depth) {
                ...
                childIds.forEach(childId => {
                    ...
                    // Link icon for child nodes (shown when link is set) - placed right after title
                    const childLink = child.details?.link?.trim();
                    if (childLink) {
                        const childLinkIcon = document.createElement('span');
                        childLinkIcon.textContent = '🔗';
                        childLinkIcon.style.cssText = 'margin-left:2px;font-size:11px;text-decoration:none;cursor:pointer;flex-shrink:0;opacity:0.7;transition:opacity 0.15s;';
                        ...
                        childLinkIcon.addEventListener('mousedown', (e) => {
                            e.stopPropagation();
                            childLinkIcon.dataset.linkPending = 'true';
                        });
                        childLinkIcon.addEventListener('mouseup', (e) => {
                            e.stopPropagation();
                            if (childLinkIcon.dataset.linkPending === 'true') {
                                window.open(childLink, '_blank', 'noopener,noreferrer');
                            }
                            childLinkIcon.dataset.linkPending = '';
                        });
                        childLinkIcon.addEventListener('mouseleave', () => {
                            childLinkIcon.dataset.linkPending = '';
                        });
            ...
            state.nodes.forEach(node => {
                ...
                // Link icon (shown when link is set) - placed right after title
                const link = node.details?.link?.trim();
                if (link) {
                    const linkIcon = document.createElement('span');
                    linkIcon.textContent = '🔗';
                    linkIcon.style.cssText = 'margin-left:2px;font-size:13px;text-decoration:none;cursor:pointer;flex-shrink:0;opacity:0.7;transition:opacity 0.15s;';
        ...
        function renderTimeline() {
            ...
            container.style.width = Math.max(totalWidth + 200, window.innerWidth) + 'px';

            // Date header
            const header = document.createElement('div');
            header.className = 'timeline-header';
            header.style.width = Math.max(totalWidth + 200, window.innerWidth) + 'px';
            for (let i = 0; i < days; i++) {
                const d = new Date(viewStart);
                d.setDate(d.getDate() + i);
                const dayEl = document.createElement('div');
                dayEl.className = 'timeline-day-header';
                dayEl.style.width = dayWidth + 'px';
                const dayNames = ['日', '月', '火', '水', '木', '金', '土'];
                const isWeekend = d.getDay() === 0 || d.getDay() === 6;
                dayEl.innerHTML = `<div>${d.getMonth() + 1}/${d.getDate()}<br><small style="color:${isWeekend ? '#e74c3c' : '#666'}">${dayNames[d.getDay()]}</small></div>`;
                if (isWeekend) dayEl.style.background = 'rgba(231,76,60,0.05)';
                header.appendChild(dayEl);
            }
            container.appendChild(header);

            // Lane definitions - include ALL nodes based on their when data
            const laneDefs = [
                { key: 'daily', label: '毎日', match: n => n.details?.when?.repeatType === 'daily' },
                { key: 'weekly', label: '毎週', match: n => n.details?.when?.repeatType === 'weekly' },
                { key: 'monthly', label: '毎月', match: n => {
                    const rt = n.details?.when?.repeatType;
                    return rt === 'monthly-day' || rt === 'monthly-week';
                }},
                { key: 'yearly', label: '毎年', match: n => {
                    const rt = n.details?.when?.repeatType;
                    return rt === 'yearly-date' || rt === 'yearly-week';
                }},
                { key: 'asap', label: '随時', match: n => {
                    const rt = n.details?.when?.repeatType;
                    return rt === 'none' && n.details?.when?.deadlineDate;
                }},
                { key: 'unset', label: '未設定', match: n => {
                    const rt = n.details?.when?.repeatType;
                    return !rt || rt === 'none' && !n.details?.when?.deadlineDate;
                }}
            ];
            ...
            laneDefs.forEach(laneDef => {
                const laneNodes = state.nodes.filter(n => laneDef.match(n));
                if (state.hideCompleted) {
                    laneNodes = laneNodes.filter(n => !n.completed);
                }

                const laneEl = document.createElement('div');
                laneEl.className = 'timeline-lane' + (timeline.expandedLanes[laneDef.key] ? '' : ' collapsed');

                // Lane header
                const laneHeader = document.createElement('div');
                laneHeader.className = 'timeline-lane-header';
                laneHeader.textContent = `${laneDef.label} (${laneNodes.length})`;
                laneHeader.addEventListener('click', () => {
                    timeline.expandedLanes[laneDef.key] = !timeline.expandedLanes[laneDef.key];
                    renderNodes();
                });
                ...
                // Lane content
                const content = document.createElement('div');
                content.className = 'timeline-lane-content';
                content.style.minHeight = timeline.laneHeight + 'px';
                ...
                for (let i = 0; i < days; i++) {
                    ...
                    line.className = 'timeline-grid-line' + ((d.getDay() === 0 || d.getDay() === 6) ? ' weekend' : '');
                ...
                if (laneNodes.length === 0) {
                    ...
                    empty.className = 'timeline-empty-lane';
                    empty.textContent = 'ノードがありません';
                    content.appendChild(empty);
                } else {
                    // Sort by date
                    laneNodes.sort((a, b) => {
                        const da = getNodeTimelineDate(a);
                        const db = getNodeTimelineDate(b);
                        if (!da && !db) return 0;
                        if (!da) return 1;
                        if (!db) return -1;
                    ...
                    laneNodes.forEach((node, index) => {
                        ...
                        const startX = startDate ? ((new Date(startDate) - viewStart) / (1000 * 60 * 60 * 24)) * dayWidth : 0;
                        const duration = startDate && endDate ? Math.max(1, (new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24)) : 1;
                        const nodeWidth = Math.max(100, duration * dayWidth - 16); // minimum 100px, minus margin

                        const wrapper = document.createElement('div');
                        wrapper.className = 'timeline-node';
                        wrapper.style.left = (startX + 8) + 'px';
                        wrapper.style.top = (index * 56) + 'px';
                        wrapper.style.width = nodeWidth + 'px';
                        wrapper.dataset.id = node.id;

                        // Render node element (timeline version, no children nesting)
                        const nodeEl = createTimelineNodeElement(node);
            ...
            container.addEventListener('mousedown', (e) => {
                ...
                if (e.target.closest('.timeline-node') || e.target.closest('.timeline-lane-header')) return;
            ...
            // Update canvas size
            canvas.style.width = Math.max(totalWidth + 200, window.innerWidth) + 'px';
        ...
        function createTimelineNodeElement(node, isChild = false) {
            const el = document.createElement('div');
            el.className = 'node' + (node.completed ? ' completed' : '') + (isChild ? ' child-node' : '');
            el.dataset.id = node.id;
            ...
            const color = document.createElement('div');
            color.className = 'node-color';
            color.style.cssText = `width:14px;height:14px;border-radius:3px;background-color:${node.color};cursor:pointer;flex-shrink:0;`;
            color.textContent = node.completed ? '✓' : '';
            ...
            if (link) {
                const linkIcon = document.createElement('span');
                linkIcon.textContent = '🔗';
                linkIcon.style.cssText = 'margin-left:2px;font-size:12px;cursor:pointer;opacity:0.7;';
                linkIcon.addEventListener('mousedown', (e) => {
                    e.stopPropagation();
                    window.open(link, '_blank', 'noopener,noreferrer');
                });
            ...
            if (!isChild && node.details?.when?.repeatType && node.details.when.repeatType !== 'none') {
                ...
                if (whenText) {
                    const preview = document.createElement('div');
                    preview.className = 'when-preview';
                    preview.textContent = whenText;
                    preview.style.cssText = 'font-size:11px;color:#666;padding:2px 8px;border-top:1px solid #eee;';
                ...
                // 2. Reorder: insert before/after target node
                    ...
                    if (targetNode) {
                        ...
                        if (targetParentId === null) {
                            ...
                            state.dragNode.width = calculateNodeWidth(state.dragNode.title, !!state.dragNode.details?.link);
        ...
        const nProject = createNode(100, 80, 'プロジェクト管理');
        ...
        // Child: 週次MTG (weekly, Monday)
        const nWeekly = createNode(0, 0, '週次MTG');
        nWeekly.color = '#64b5f6';
        nWeekly.priority = 3;
        nWeekly.parentId = nProject.id;
        nProject.children.push(nWeekly.id);
        nWeekly.details.when = {
            repeatType: 'weekly',
            daysOfWeek: [1],
        ...
        nWeekly.details.what = '週次進捗確認と次週計画';
        nWeekly.details.who = '全員';
        nWeekly.details.where = '会議室A';

        // Child: 月次レポート
        const nMonthly = createNode(0, 0, '月次レポート');
        nMonthly.color = '#7986cb';
        nMonthly.priority = 4;
        nMonthly.parentId = nProject.id;
        nProject.children.push(nMonthly.id);
        nMonthly.details.when = {
        ...
        nMonthly.details.what = '月次業務レポート作成・提出';

        // Child: 日次確認
        const nDaily = createNode(0, 0, '日次確認');
        nDaily.color = '#4dd0e1';
        nDaily.priority = 2;
        nDaily.parentId = nProject.id;
        nProject.children.push(nDaily.id);
        nDaily.details.when = {
            repeatType: 'daily',
        ...
        nDaily.details.what = '前日実績確認と本日予定の見直し';

        // Root 2: 開発タスク (green)
        const nDev = createNode(500, 60, '開発タスク');
        nDev.color = '#81c784';
        nDev.priority = 5;
        ...
        // Child: フロントエンド実装
        const nFront = createNode(0, 0, 'フロントエンド実装');
        nFront.color = '#aed581';
        nFront.priority = 4;
        nFront.parentId = nDev.id;
        nDev.children.push(nFront.id);
        nFront.details.when = {
            repeatType: 'none',
            daysOfWeek: [],
            baseDateTime: null,
        ...
        nFront.details.link = 'https://developer.mozilla.org';
        nFront.details.what = 'UIコンポーネント実装・スタイル調整';

        // Child: バックエンドAPI
        const nBack = createNode(0, 0, 'バックエンドAPI');
        nBack.color = '#dce775';
        nBack.priority = 4;
        nBack.parentId = nDev.id;
        nDev.children.push(nBack.id);
        nBack.details.when = {
            repeatType: 'none',
            daysOfWeek: [],
            baseDateTime: null,
        ...
        nBack.details.what = 'REST API設計・実装・テスト';

        // Child: テスト作成
        const nTest = createNode(0, 0, 'テスト作成');
        nTest.color = '#fff176';
        nTest.priority = 3;
        nTest.parentId = nDev.id;
        nDev.children.push(nTest.id);
        nTest.details.what = '単体テスト・結合テスト作成';

        // Root 3: リマインダー (yellow)
        const nRemind = createNode(900, 80, 'リマインダー');
        nRemind.color = '#ffd54f';
        nRemind.priority = 3;
        nRemind.expanded = true;

        // Child: 健康診断
        const nHealth = createNode(0, 0, '健康診断');
        nHealth.color = '#ffb74d';
        nHealth.priority = 5;
        nHealth.parentId = nRemind.id;
        nRemind.children.push(nHealth.id);
        ...
        // Child: 備品発注
        const nSupply = createNode(0, 0, '備品発注');
        nSupply.color = '#ff8a65';
        nSupply.priority = 2;
        nSupply.parentId = nRemind.id;
        nRemind.children.push(nSupply.id);
        ...
        // Root 4: 随時対応 (red)
        const nAsap = createNode(100, 400, '随時対応');
        nAsap.color = '#e57373';
        nAsap.priority = 4;
        nAsap.expanded = true;

        // Child: 問い合わせ対応
        const nInquiry = createNode(0, 0, '問い合わせ対応');
        nInquiry.color = '#f06292';
        nInquiry.priority = 3;
        nInquiry.parentId = nAsap.id;
        nAsap.children.push(nInquiry.id);
        nInquiry.details.what = '顧客からの問い合わせ対応';

        // Child: バグ修正 (urgent deadline)
        const nBug = createNode(0, 0, 'バグ修正 #123');
        nBug.color = '#ba68c8';
        nBug.priority = 5;
        nBug.parentId = nAsap.id;
        nAsap.children.push(nBug.id);
        nBug.details.when = {
            repeatType: 'none',
            daysOfWeek: [],
            baseDateTime: null,
        ...
        nBug.details.link = 'https://github.com/00Ituki00/taskmind/issues/123';
        nBug.details.what = '接続線が重複表示されるバグの修正';

        // Child: ドキュメント更新 (completed)
        const nDoc = createNode(0, 0, 'ドキュメント更新');
        nDoc.color = '#9575cd';
        nDoc.priority = 2;
        nDoc.parentId = nAsap.id;
        nAsap.children.push(nDoc.id);
        nDoc.completed = true;
        nDoc.completedAt = '2026-05-18T10:00:00.000Z';
        nDoc.details.what = 'READMEと設計書の更新';

        // Root 5: 未設定タスク
        const nUnset = createNode(900, 400, '未設定タスク');
        nUnset.color = '#a1887f';
        nUnset.priority = 1;
        nUnset.details.what = 'アイデア置き場';

        // Connections (between root nodes only)
        createConnection(nProject.id, nDev.id, '関連');
        createConnection(nDev.id, nAsap.id, '派生');
        createConnection(nAsap.id, nRemind.id, '参考');
